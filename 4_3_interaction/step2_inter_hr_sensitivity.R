...
data$age_std <- scale(data$age_recruit,center=T,scale=T)
...
lipids_list2 <- lipids_list[c(3,5,7,7,8,11,
			      ...
			      18,19,19,
			      20,20,20,21,
			      23)] # traits significant in step 1


lipids_list2_prs <- c('Lipids','Lipids','Lipids','Respir','BP','Respir',
                      'BP','Respir',
                      'BP','Respir',
                      'BP','Respir','T2D',
                      'BP','BP','Obesity','Respir',
                      'Respir','Obesity','Respir','Immune','Obesity','Respir',
                      'Respir','Respir') # prs significant in step 1; sequence matched to the traits in lipids_list2

for(i in 1:length(lipids_list2)){
  add <- getINT_cont(lipids_list2[i],
		     lipids_list2_prs[i])
...
binary_list2 <- binary_list[c(2,3,3,
                              8,8,9)]  # traits significant in step 1
binary_list2_prs <- c('Respir','Respir','T2D',
                      'Respir','T2D','Respir') # prs significant in step 1; sequence matched to the traits in binary_list2
for(i in 1:length(binary_list2)){
  add <- getINT_bi(binary_list2[i],
		   binary_list2_prs[i])
  data <- merge(data,add,by='eid',all.x=T)
}
...
phenotype <- c(paste0('trans_',lipids_list2,'_',lipids_list2_prs),
	       paste0('trans_',binary_list2,'_',binary_list2_prs))
phe_prs <- c(lipids_list2_prs,binary_list2_prs)

inter_out <- data.frame()
for(i in 1:length(phenotype)){
  inter_out <- rbind(inter_out,
	getInter(phenotype[i],phe_prs[i]))
  print(phenotype[i])
}
inter_out <- cbind.data.frame(phenotype,inter_out)
colnames(inter_out) <- c('trait_prs','p-value')


# output
write.csv(inter_out,
	  '/gpfs/gibbs/pi/zhao/jh2875/ps_PRS/clean/data/prs_analysis_V2_absolute//interaction_trans_selected_psprs_adj.csv',
  	  row.names=T)
