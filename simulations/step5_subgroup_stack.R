...
ps_names <- paste0('simu_PRS_Observed_Beta_Trait1_',1:4)
...
sum_psprs <- apply(abs(dat.high[,15:18]),1,sum) # sum of 4 PD-PRSs
# absolute value of overall PRS explained by PD-PRS
prop <- apply(abs(dat.high[,15:18]),2,
                function(x){(abs(x)/sum_psprs)*dat.high$simu_PRS_Observed_Beta_Trait1_all})
...
## rank by CAD PRS
iid <- dat.high$IID[order(dat.high$simu_PRS_Observed_Beta_Trait1_all,decreasing=T)]
...
colnames(mean_contri) <- paste0('subgroup',1:4)
...
names(color) <- paste0('simu_PRS_Observed_Beta_Trait1_',1:4)
### label for each psPRS
label <- c(paste0('Trait ',2:4,'-related PD-PRS'),
	   'Others PD-PRS')
names(label) <- paste0('simu_PRS_Observed_Beta_Trait1_',1:4)
...
for(i in 1:4){
  ...
  ylab.name <- ifelse(i==4,'Others Subgroup',
		      paste0('Trait ',i+1,'-related Subgroup'))
