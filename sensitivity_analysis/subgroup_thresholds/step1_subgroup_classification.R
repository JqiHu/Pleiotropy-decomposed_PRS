# classify top ?% CAD PRS into 9 groups
setwd('/gpfs/gibbs/pi/zhao/jh2875/ps_PRS/review/PLOS_CB/subgroup_sensitivity/data')
library('data.table')
library('stringr')
rm(list=ls())

#### set the threshold for subgroup
args <- commandArgs(trailingOnly=T)
threshold=as.numeric(args[1]) # 0.1 or 0.01
setwd(as.character(threshold))
...
## top PD-PRSs in general population
cutoffs <- apply(prs[,-c(1,2)],2,
		 function(x){quantile(x,1-threshold)})

## identify top CAD PRS
prs_high <- prs[which(prs$CAD > 
                        quantile(prs$CAD,1-threshold)),] # extract top 5% risk subjects for CAD
prs_high <- apply(prs_high,2,as.numeric)
prs_high <- as.data.frame(prs_high)

saveRDS(prs_high,paste0('high_risk_ind.RDS'))
